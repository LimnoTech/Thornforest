...
- **Never run `git commit`.** Commits are made manually by the user in GitHub Desktop. Each task ends at a verified checkpoint; leave changes staged/on-disk for the user.
...
Leave the new `1_usgs_hydrofabric.{py,ipynb}` and the staged deletion for the user to commit.
...
- [ ] **Step 3: Checkpoint** — leave the updated NB2 for the user.
...
- [ ] **Step 4: Checkpoint** — leave all changes (`CLAUDE.md`, `README.md`, `_freeze/`, `_site/` is git-ignored) for the user to review and commit. Remind them of the Task 7 manual GitHub steps before the first deploy.
